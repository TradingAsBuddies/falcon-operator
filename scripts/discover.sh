...
# Database node
echo ">>> Discovering falcon-db (192.168.1.106)..."
if ssh -o ConnectTimeout=5 -o BatchMode=yes 192.168.1.106 "echo connected" &>/dev/null; then
    echo "  [OK] SSH connection successful"

    echo "  Checking PostgreSQL..."
    ssh 192.168.1.106 "sudo systemctl status postgresql --no-pager 2>/dev/null || echo 'PostgreSQL not running as systemd service'" | head -5

    echo "  Checking disk usage..."
    ssh 192.168.1.106 "df -h / | tail -1"

    echo "  Finding application directories..."
    ssh 192.168.1.106 "ls -la ~/Projects 2>/dev/null || ls -la ~/ | grep -E '^d'" | head -10

    echo "  Checking running Python processes..."
    ssh 192.168.1.106 "ps aux | grep -E 'python|node' | grep -v grep" | head -5
else
    echo "  [FAIL] Cannot connect via SSH - run: ssh-copy-id 192.168.1.106"
fi
