#!/usr/bin/env python3
"""
Final production backtest for One Candle strategy with intraday data
"""
import sys
...
def load_intraday_data(ticker, days):
    ...
    print(f"Loaded {len(df)} bars ({df['datetime'].dt.date.nunique()} trading days)")
    return df
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"ONE CANDLE RULE - PRODUCTION BACKTEST (Intraday Data)")
    print(f"{'='*80}")
    ...
    # Use relaxed confirmation for 1-minute data
    cerebro.addstrategy(
        OneCandleStrategy,
        lookback_period=20,
        ...
        require_confirmation=False,  # Too strict for 1-min bars
        stop_loss_pct=0.02,
        trade_start_hour=9,
    ...
    print(f"\nNote: Confirmation disabled for 1-min bars (too strict)")
    print(f"Consider using 5-minute bars or adjusting min_body_size")
    print(f"{'='*80}\n")
